...
def save_cache(
    ...
    all_models = sorted(set(chat_models + code_models))
    data = {
        "version": 2,
        "saved_at": time.time(),
    ...
    try:
        ...
        logger.info(
            "[ModelCache] Saved: %d chat, %d code, %d total",
            len(chat_models), len(code_models), len(all_models),
        )
    except Exception as exc:
...
def get_full_cache() -> dict[str, Any]:
    ...
    # Prefix all model names for consistent API output
    chat = _prefix_list(cache.get("chat_models", []))
    code = _prefix_list(cache.get("code_models", []))
    all_m = _prefix_list(cache.get("all_models", []))
    # Capabilities and modes keyed by bare name — keep as-is (frontend strips prefix)
    return {
        "chat_models": chat,
        "code_models": code,
        "all_models": all_m,
